import type { ChromeTab } from "./tab-actions"
...
const URL_PATTERNS = {
  ...
  ignoredSubdomains: new Set(["www", "m", "mobile", "shop"])
}
 ...
 */
export async function getAllBookmarks(): Promise<NormalizedBookmark[]> {
  return new Promise((resolve) => {
    chrome.bookmarks.getTree((bookmarkTreeNodes) => {
      const collectedBookmarks: NormalizedBookmark[] = []

      function traverseBookmarkTree(nodes: chrome.bookmarks.BookmarkTreeNode[]) {
 ...
 */
export function findBookmarkedTabs(
  tabs: ChromeTab[],
  normalizedBookmarkUrls: string[]
): ChromeTab[] {
 ...
 */
export function findBlankTabs(tabs: ChromeTab[]): ChromeTab[] {
  return tabs.filter((tab) => {
