import { getSafeUrls, getNeverCloseUrls, getSetting, STORAGE_KEYS } from '@/lib/storage';
import { findMatchingTabs, getDuplicateTabIds } from '@/lib/tab-actions';
import { matchesUrlPattern } from '@/lib/url-utils';
import { getAllBookmarks, findBookmarkedTabs, findBlankTabs } from '@/lib/bookmark-utils';
import { tileAllTabs } from '@/lib/window-tiling';
import { setBadgeBackgroundColor, setBadgeText } from '@/lib/browser-api';
...
export default defineBackground(() => {
  ...
  function isNeverCloseTab(tab: browser.Tabs.Tab, neverCloseUrls: string[]): boolean {
    if (!tab?.url || !neverCloseUrls.length) return false;
    return neverCloseUrls.some((pattern) => matchesUrlPattern(tab.url!, String(pattern || '')));
  }

  async function getTabIdsToClose(): Promise<Set<number>> {
  ...
  async function handleCloseTabs(): Promise<number> {
    try {
      ...
      await browser.tabs.remove(Array.from(tabIds));
      return tabIds.size;
    } catch (error) {
      console.error('Error closing tabs:', error);
      return 0;
