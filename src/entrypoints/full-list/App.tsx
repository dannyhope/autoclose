...
import { getSafeUrls } from '@/lib/storage';
import { matchesUrlPattern, parseUrlParts } from '@/lib/url-utils';
...
export function FullListApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [safeUrls, tabs] = await Promise.all([getSafeUrls(), browser.tabs.query({})]);
        if (!safeUrls.length) {
