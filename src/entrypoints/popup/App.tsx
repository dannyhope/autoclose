...
import { matchesUrlPattern, toPatternFromTabUrl } from '@/lib/url-utils';
import { findMatchingTabs, getDuplicateTabIds } from '@/lib/tab-actions';
import { getUIState, setUIState, UI_STATE_KEYS } from '@/lib/ui-state';
import { getAllBookmarks, findBookmarkedTabs, findBlankTabs } from '@/lib/bookmark-utils';
...
export function App() {
  ...
  const clearHighlights = useCallback(() => {
    highlightedIds.current.forEach((id) => {
      browser.tabs.sendMessage(id, { action: 'setTabWarning', enabled: false });
    });
  ...
  const refresh = useCallback(async () => {
    ...
    if (await getUIState(UI_STATE_KEYS.ALWAYS_CLOSE_BOOKMARKED)) {
      ...
      const bookmarkUrls = bookmarks.map((bookmark) => bookmark.normalizedUrl);
      findBookmarkedTabs(tabs, bookmarkUrls).forEach((tab) => {
        if (typeof tab.id === 'number') {
          nextHighlighted.add(tab.id);
        }
      });
      findBlankTabs(tabs).forEach((tab) => {
        if (typeof tab.id === 'number') {
          nextHighlighted.add(tab.id);
        }
      });
  ...
  useEffect(() => {
    ...
    browser.tabs.onUpdated.addListener(debounced);
    browser.tabs.onRemoved.addListener(debounced);
    browser.tabs.onCreated.addListener(debounced);
    return () => {
      cancelled = true;
      document.removeEventListener('keydown', onKeyDown);
      document.removeEventListener('keyup', onKeyUp);
      document.removeEventListener('visibilitychange', onVisibility);
      browser.tabs.onUpdated.removeListener(debounced);
      browser.tabs.onRemoved.removeListener(debounced);
      browser.tabs.onCreated.removeListener(debounced);
      clearHighlights();
    };
  ...
  async function handleTileTabs() {
    ...
    setTileConfirm(null);
    await browser.runtime.sendMessage({ action: 'tileTabs' });
    window.close();
  }
