...
function stripCommonUrlParts(url: URL): string {
...
export function normalizeBookmarkUrl(urlString: string): string {
  try {
...
export async function getAllBookmarks(): Promise<Bookmark[]> {
  try {
    ...
    function traverseBookmarkTree(nodes: browser.Bookmarks.BookmarkTreeNode[]): void {
    ...
    return collectedBookmarks;
  } catch (error) {
    console.warn('Bookmarks are unavailable in this browser:', error);
    return [];
  }
}

export function findBookmarkedTabs(tabs: browser.Tabs.Tab[], normalizedBookmarkUrls: string[]): browser.Tabs.Tab[] {
...
export function findBlankTabs(tabs: browser.Tabs.Tab[]): browser.Tabs.Tab[] {
  return tabs.filter(tab => {
    const isBlankTab = tab.url === 'about:blank'
      || tab.url === 'chrome://newtab/'
      || tab.url === 'about:newtab'
      || tab.url === 'safari://newtab/'
      || (tab as any).pendingUrl === 'about:blank'
      || (tab as any).pendingUrl === 'chrome://newtab/'
      || (tab as any).pendingUrl === 'about:newtab';
    return isBlankTab;
  });
}
